...
public class ColorPoller implements Runnable {
  ...
  public void run() {
    ...
    while (!kill) {
      ...
      if (!leftLineDetected) { // If we have already detected a line, don't try to detect it again because it will give
                               // a false negative
        leftLineDetected = leftDetectBlackLine();
      }
      if (!rightLineDetected) { // If we have already detected a line, don't try to detect it again because it will give
                                // a false negative
        rightLineDetected = rightDetectBlackLine();
      }

      if ((leftLineDetected && rightLineDetected) || !(leftLineDetected && rightLineDetected)) { // Both lines detected,
                                                                                                 // (or no detections),
                                                                                                 // carry on
      ...
      } else if (leftLineDetected && !rightLineDetected) { // Left side detection, stop left motor keep right running
        leftMotor.stop(true);
      } else if (rightLineDetected && !leftLineDetected) { // Right side detection, stop right motor keep left running
  ...
  /**
   *
   * Reset the X position based on what tile the robot thinks it is on, if the robot is less than half a tile
   * away from its current position in each direction, correction is possible
   *
   * @param currXYT
   * @param updatedTheta
   */
  ...
  /**
  *
  * Reset the Y position based on what tile the robot thinks it is on, if the robot is less than half a tile
  * away from its current position in each direction, correction is possible
  *
  * @param currXYT
  * @param updatedTheta
